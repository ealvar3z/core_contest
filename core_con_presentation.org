#TITLE: Hallucination as Non-Identifiability Under Forced Action
#AUTHOR: E. Alvarez
...
- *Next-token model*
  ...
  P(x_{t1} | x_{\le t}) and generates text by sampling or decoding from it.
...
Let:
- W = {w₁, w₂} be two distinct world states.
...
There exists a proposition φ such that:
- φ is true in w₁
- φ is false in w₂
- the prompt x_{\le t} does not encode which world holds

(S3) *Next-token learner*
A model M is trained to approximate P(x_{t1} | x_{\le t}) from samples of the observational channel.

(S4) *Forced emission*
Given x_{\le t}, the model must emit a continuation asserting either φ or ¬φ.

** Lemma 1: Irreducible posterior uncertainty

Because the observational channel is identical,

P(w₁ | x_{\le t}) = P(w₂ | x_{\le t})
...
#begin_example
   ...
   (φ is true)                  (φ is false)
     ...
     assert φ                assert ¬φ
        |                         |
  false in w₂               false in w₁
#end_example

* Consequences

- Scaling model size or compute improves approximation of P(x_{t1} | x_{\le t}) but does not restore identifiability.
...
* Spoken Proof (My Recording Script)
...
Consider two distinct world states, w₁ and w₂, which differ in a factual property but induce identical text distributions.
For all observable texts x, P(x | w₁) equals P(x | w₂).
...
A next-token model trained on this data learns P(x_{t1} | x_{\le t}).
Because the observational channel is identical, the posterior over worlds remains equal for every prompt.

Now consider an underspecified prompt and a proposition φ that is true in w₁ and false in w₂.
At inference time, the model must answer.
If it asserts φ, it is wrong in w₂.
If it asserts not φ, it is wrong in w₁.
